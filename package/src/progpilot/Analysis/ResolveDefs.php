...
class ResolveDefs {
	...
	public static function instance_build_back($data, $myfunc, $myfunc_call)
	{
        if(is_null($myfunc))
        {
            if($myfunc_call->is_instance())
            {
                ...
                $new_myclass = $myfunc_call->get_back_def();
                
                $instances_pre = ResolveDefs::select_instances_ins($data, $mydef, true);
                $instances = Definitions::unique_nearest_byblock($myfunc_call->getLine(), $myfunc_call->getColumn(), $instances_pre);
                
                foreach($instances as $instance)
                {
                echo "instance_build_back foreach instance\n";
                $instance->print_stdout();
                    ...
                    foreach($myclass->get_methods() as $method)
                    {
                        ...
                        $new_myclass->add_property($new_method);
                    }
                }
            }
        }
        
        else if(!is_null($myfunc) && $myfunc->is_method())
        {
            if($myfunc_call->is_instance())
            {
    ...
    public static function select_instances_ins($data, $tempdefa, $check = false)
	{
        $instances_defs = [];
        ...
        if($tempdefa->is_property() || $tempdefa->is_method() || $check)
        {
            ...
            $instances_defs = Definitions::search_nearest(
                    ...
                    $data->getin($copy_tempdefa->get_block_id()), 
                    $copy_tempdefa);
        }
		...
		if(count($defs_found) > 0)
		{
			...
			{	
				if($defa->is_ref())
				{
                    ...
                    $defs_found = Definitions::unique_nearest_byblock($refdef->getLine(), $refdef->getColumn(), $truerefs);
