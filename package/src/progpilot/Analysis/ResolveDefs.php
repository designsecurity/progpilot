...
class ResolveDefs
{
    ...
    public static function funccall_class($context, $data, $myfunc_call)
    {
        ...
        } elseif ($myfunc_call->is_type(MyFunction::TYPE_FUNC_METHOD)) {
                ...
                foreach ($copy_myclass->get_properties() as $property) {
                    ...
                    foreach ($properties_inside as $property_inside) {
                        ...
                        if ($property_inside->is_type(MyDefinition::TYPE_INSTANCE) && !$new_property->is_type(MyDefinition::TYPE_INSTANCE)) {
                            ...
                            $myclass = $context->get_objects()->get_myclass_from_object($property_inside->get_object_id());
                            /*
                            echo "objectif = '".$property_inside->get_object_id()."' classname = '".$myclass->get_name()."'\n";
                            $property_inside->print_stdout();

                            echo "new_property =============================================>\n";
                            $new_property->print_stdout();
                            */
                        }
                    }
    ...
    public static function select_definitions($context, $data, $defsearch, $bypass_isnearest = false)
    {
        ...
        foreach ($data as $def) {
            if (Definitions::def_equality($def, $defsearch, $bypass_isnearest)
                ...
                } elseif (($def->is_type(MyDefinition::TYPE_PROPERTY) === $defsearch->is_type(MyDefinition::TYPE_PROPERTY))
                ...
                elseif (!$def->is_type(MyDefinition::TYPE_INSTANCE) && $defsearch->is_type(MyDefinition::TYPE_PROPERTY)) {
                    $defsfound[$def->get_block_id()][] = $def;
                }
            }
        }
